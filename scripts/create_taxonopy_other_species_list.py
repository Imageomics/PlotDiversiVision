...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Extract unique plant names for one column in the provided "
    ...
    parser.add_argument(
        "--source-csv",
        type=Path,
        default=Path("assets/CPER_BONAPlist.csv"),
    )
    parser.add_argument("--column", required=True, default="scientific_name",
                        help="Column name, for example scientific_name.")
    parser.add_argument(
        "--name",
        default="CPER_BONAPlist",
        help="Output dataset stem.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def extract_species(args: argparse.Namespace, output_dir: Path) -> tuple[Path, int, int]:
    ...
    with args.source_csv.open(newline="", encoding="utf-8-sig") as f:
        ...
        for row in reader:
            rows_seen += 1
            label = clean(row.get(target_column))
            if not label:
                continue
            labels.add(label)
            rows_used += 1
    ...
    if not labels:
        raise ValueError(f"No species labels found for column '{args.column}'")
...
def main() -> int:
    ...
    print(f"Rows seen for {args.column}: {rows_seen}")
